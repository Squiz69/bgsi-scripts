...
local timerStartTime = 0 -- Added to fix timer issue
...
-- Find and press play button with improved detection
if LocalPlayer and LocalPlayer:FindFirstChild("PlayerGui") then
for _, gui in pairs(LocalPlayer.PlayerGui:GetDescendants()) do
if (gui:IsA("TextButton") or gui:IsA("ImageButton")) and gui.Visible then
local buttonText = ""
if gui:IsA("TextButton") then
buttonText = string.lower(gui.Text)
elseif gui:FindFirstChild("TextLabel") and gui.TextLabel:IsA("TextLabel") then
buttonText = string.lower(gui.TextLabel.Text)
end

if buttonText:match("play") or buttonText:match("start") or buttonText:match("begin") then
pcall(function()
...
if buttonText:match("optimized") or buttonText:match("optimize") or buttonText:match("performance") then
pcall(function()
gui.MouseButton1Click:Fire()
end)
...
-- Look for any popup dialog and close it
wait(1)
for _, gui in pairs(LocalPlayer.PlayerGui:GetDescendants()) do
if gui:IsA("Frame") and gui.Visible and (gui.Name:lower():match("dialog") or gui.Name:lower():match("popup")) then
local closeButton = gui:FindFirstChild("CloseButton") or gui:FindFirstChild("Close")
...
-- Fixed Discord webhook notification function with proper error handling and verification
local function sendDiscordNotification(eggInfo, eggLuck)
if not eggInfo then return end

-- Verify webhook URL is valid
if not DISCORD_WEBHOOK_URL:match("^https://discord.com/api/webhooks/") then
...
local timeRemaining = "~60 seconds"

-- Create webhook content with proper structure
local webhookData = {
content = "🔥 25X Luck Egg Found! Join quickly! 🔥",
embeds = {
{
...
footer = {text = "25x Egg Finder v3.3"}
}
}
}
...
spawn(function()
for attempt = 1, 5 do -- Increased retry attempts
...
print("Webhook attempt " .. attempt .. " failed, retrying... Error: " .. tostring(response))
wait(1.5) -- Slightly increased wait time between retries
end
end
...
if depth > 5 then return {} end -- Increased depth for more thorough scanning
...
-- Check for numeric attributes with "luck" in name
for attrName, value in pairs(obj:GetAttributes()) do
local attrNameLower = string.lower(attrName)
if (string.find(attrNameLower, "luck") or 
string.find(attrNameLower, "multi") or 
string.find(attrNameLower, "bonus")) then
...
-- Check if it has egg-like appearance (spherical or oval shape)
if obj:IsA("MeshPart") and obj.MeshId and 
(string.find(obj.MeshId:lower(), "egg") or 
string.find(obj.MeshId:lower(), "sphere") or 
string.find(obj.MeshId:lower(), "oval")) then
isEgg = true
end
...
-- Quick server hop (but won't hop if an egg was found recently)
...
timerStartTime = tick() -- Reset timer start time
return false
end
...
-- Optimized main loop with improved egg handling and fixed timer
local function mainLoop()
if isSearching then return end
isSearching = true

print("------- 25x Egg Finder v3.3 Started -------")
...
-- Reset timer start time to stay in this server longer
timerStartTime = tick()
wait(1)
end
else
-- Use fixed timer calculation
local elapsedSearchTime = tick() - timerStartTime

-- Don't hop if we found an egg recently
if elapsedSearchTime > maxServerSearchTime and not eggFoundRecently then
print("Max search time reached (" .. maxServerSearchTime .. "s), hopping to next server")
if hopToNextServer() then
wait(2)
timerStartTime = tick() -- Reset timer when hopping
notifiedEggs = {}
pressPlayButtons()
wait(2)
end
...
title.TextSize = 16
title.Text = "25x Egg Finder v3.3"
title.Parent = frame
...
-- Fixed status update with non-resetting timer
spawn(function()
while wait(0.5) do
if not screenGui or not screenGui.Parent then return end

local elapsedTime = tick() - timerStartTime
local timeLeft = math.max(0, maxServerSearchTime - elapsedTime)
