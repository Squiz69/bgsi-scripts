...
local tweenSpeed = 8.0          -- Speed for tweening to eggs (increased to 8)
...
-- Improved tweening function with proper height handling
local function tweenToEgg(egg)
    if not egg then 
        print("Tweening failed: No egg provided")
        return false 
    end
    
    print("Attempting to tween to priority egg: " .. egg.Name)
    ...
    if egg:IsA("BasePart") then
        ...
        -- Look for primary parts first
        eggPart = egg:FindFirstChild("PrimaryPart") or egg:FindFirstChildWhichIsA("BasePart")
    ...
    if not eggPart then 
        print("Tweening failed: Could not find a valid part in the egg")
        return false 
    end
    ...
    if not character then 
        print("Tweening failed: Character not found")
        return false 
    end
    ...
    if not humanoidRootPart or not humanoid then 
        print("Tweening failed: HumanoidRootPart or Humanoid not found")
        return false 
    end
    ...
    if currentTween then
        ...
        currentTween = nil
        wait(0.1) -- Brief pause to ensure cancellation completes
    end
    
    -- Calculate tween position (allow some height difference but not too much)
    local targetY = eggPart.Position.Y
    local characterY = humanoidRootPart.Position.Y
    
    -- Adjust Y position to be reasonable (no more than 10 studs difference)
    local finalY
    if math.abs(targetY - characterY) > 10 then
        -- If egg is too high or too low, get as close as reasonable
        finalY = characterY + (targetY > characterY and 5 or -2)
    else
        -- Otherwise go to the egg's height
        finalY = targetY
    end
    ...
    -- Prevent tweening if already very close
    if distance < 5 then
        print("Already close to egg, no need to tween")
        return true
    end
    
    -- Calculate tweening time based on distance and speed
    -- Cap the maximum time to 10 seconds to prevent very long tweens
    local tweenTime = math.min(distance / (tweenSpeed * 10), 10)
    
    -- Create tween info with slight easing for more natural movement
    ...
    -- Create the actual tween with offset to not collide with the egg
    local targetCFrame = CFrame.new(targetPosition) * CFrame.new(0, 0, -2)
    
    -- Create and store the tween
    local newTween = TweenService:Create(humanoidRootPart, tweenInfo, {
        CFrame = targetCFrame
    })
    
    -- Track the current tween
    currentTween = newTween
    
    -- Print debug info
    print("Starting tween to egg:")
    print("- Distance: " .. math.floor(distance*10)/10 .. " studs")
    print("- Tween time: " .. math.floor(tweenTime*10)/10 .. " seconds")
    
    -- Start the tween
    newTween:Play()
    
    -- Use both tween completion methods for reliability
    local tweenComplete = false
    
    -- Method 1: Use Completed event with timeout
    local timeoutThread = coroutine.create(function()
        local startTime = tick()
        while currentTween == newTween and tick() - startTime < tweenTime * 1.5 do
            wait(0.1)
        end
        tweenComplete = true
    end)
    ...
    -- Method 2: Wait for completed event
    pcall(function()
        newTween.Completed:Wait()
        tweenComplete = true
    end)
    
    -- Wait for either method to complete
    while not tweenComplete do
        wait(0.1)
    end
    
    -- Only clear if this is still the current tween
    if currentTween == newTween then
        currentTween = nil
    end
