...
local tweenSpeed = 8.0          -- Speed for tweening to eggs
...
-- Fixed tweening function - the main fix to ensure player tweens to egg
local function tweenToEgg(egg)
    ...
    -- Create tween to egg ground position with proper settings
    local tweenInfo = TweenInfo.new(
        distance / (tweenSpeed * 2), -- Adjusted speed for more reliable movement
    ...
    local lookAt = CFrame.lookAt(
        groundPosition,
        Vector3.new(eggPart.Position.X, groundPosition.Y, eggPart.Position.Z)
    )
    
    -- Create and store the tween
    currentTween = TweenService:Create(humanoidRootPart, tweenInfo, {
        CFrame = lookAt * CFrame.new(0, 0, -3) -- Approach from 3 studs away
    })
    
    -- Print additional logging
    print("Starting tween from: " .. tostring(humanoidRootPart.Position))
    print("Tweening to: " .. tostring(groundPosition))
    print("Distance: " .. tostring(distance) .. " studs")
    print("Estimated time: " .. tostring(distance / (tweenSpeed * 2)) .. " seconds")
    
    -- Play the tween with error handling
    local success = pcall(function()
        currentTween:Play()
    ...
    if not success then
        ...
        humanoidRootPart.CFrame = lookAt * CFrame.new(0, 0, -3)
        wait(0.5)
        return true
    end
