...
-- Improved button pressing function with better detection
local function pressPlayButtons()
    ...
    local playButtonPressed = findAndClickButton({"play", "start", "enter", "join"}, 1)
...
local function sendDiscordNotification(eggInfo, eggLuck)
        ...
        embeds = {
            {
                ...
                footer = {text = "25x Egg Finder v3.5"}
            }
        }
    }
    ...
    for attempt = 1, 3 do
        ...
        if success then
            print("Discord notification sent: " .. eggType)
...
-- Enhanced egg detection that prioritizes the egg_priority
local function findLuckyEgg()
    print("Scanning for 25x lucky eggs...")
    local eggPatterns = getEggPatterns()
    
    -- Scan for luck eggs with priority focus
    local function scanForLuckEggs(parent, depth)
        if depth > 5 then return {} end
        
        local foundEggs = {}
        local priorityEgg = nil
        
        for _, obj in pairs(parent:GetChildren()) do
            if obj ~= Players and obj.Name ~= "Players" then
                -- Check if it matches our priority egg first
                local objNameLower = string.lower(obj.Name)
                local isPriorityEgg = string.find(objNameLower, string.lower(egg_priority)) ~= nil
                
                -- Check if it has egg-like patterns in name
                local isEgg = isPriorityEgg
                ...
                -- Additional checks for hatchable eggs
                if not isEgg then
                ...
                -- Check for egg with interactive elements
                ...
                if isEgg then
                    -- Look for luck attributes and values
                    local luckValue = nil
                    ...
                    -- Check value objects if attributes not found
                    ...
                    -- UI Text scanning for luck values
                    if not luckValue then
                    ...
                    if luckValue and luckValue >= 25 then
                        if isPriorityEgg then
                            return {Egg = obj, LuckValue = luckValue}
                        end
                ...
                local childResult = scanForLuckEggs(obj, depth + 1)
                if childResult and childResult.Egg then
                    return childResult
                end
            end
        end
    ...
    -- Perform the scan on workspace first (priority)
    local result = scanForLuckEggs(workspace, 0)
    if result and result.Egg then
        return result.Egg, result.LuckValue
    end
    
    -- If nothing found in workspace, check ReplicatedStorage
    result = scanForLuckEggs(ReplicatedStorage, 0)
    if result and result.Egg then
        return result.Egg, result.LuckValue
    end
...
-- Improved tween movement that only works for valid eggs
...
local function mainLoop()
    ...
    print("------- 25x Egg Finder v3.5 Started -------")
...
title.TextSize = 16
title.Text = "25x Egg Finder v3.5"
title.Parent = frame
...
print("🚀 25x Egg Finder v3.5 initialized")
