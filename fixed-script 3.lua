...
local PLACE_ID = 16302670534  -- FIXED: Corrected place ID for Bubble Gum Simulator Infinity
...
local function sendWebhook(eggInfo, luckValue)
    ...
    local content = "**" .. displayName .. " Found!** :tada:\n" ..
                   ...
                   "Server Version\n" .. serverVersion .. "\n" ..
                   "Job ID\n" .. jobId .. "\n" ..
                   "Web Browser\n" .. "https://www.roblox.com/games/start?placeId=" .. PLACE_ID .. "&launchData=" .. jobId .. "\n" ..  -- FIXED: Corrected URL format
                   "Teleport Script\n" .. "game:GetService(\"TeleportService\"):TeleportToPlaceInstance(" .. PLACE_ID .. ", \"" .. jobId .. "\", game.Players.LocalPlayer)"  -- FIXED: Removed quotes around PLACE_ID
...
local function hopToNextServer()
    ...
    -- Get server list with proper error handling
    ...
    if not success then
        print("Error fetching server list: ", result)
        wait(3)
        return false
    end
    
    if not result or not result.data then
        print("Invalid server list data received")
        wait(3)
        return false
    end
    
    local servers = {}
    local validServerCount = 0
    
    -- Filter servers with better error handling
    for _, server in pairs(result.data) do
        if type(server) == "table" and server.id and server.playing and server.maxPlayers then
            if server.playing < server.maxPlayers and server.playing > 0 and not visitedServers[server.id] then
                ...
                validServerCount = validServerCount + 1
            end
        end
    end
    
    print("Found " .. validServerCount .. " valid servers to hop to")
    ...
    if #servers > 0 then
        ...
        if selectedServer and selectedServer.id then
            ...
            -- More robust teleport with error handling
            ...
            print("Selected server data is invalid")
        end
    else
        print("No suitable servers found, clearing visited servers list")
        -- Reset visited servers if we can't find any
        visitedServers = {}
    end
...
local function mainLoop()
    ...
    print("------- 25x Priority Egg Finder v5.2 Started -------")
    print("Priority egg: " .. egg_priority)
    print("Game ID: " .. PLACE_ID .. " (Bubble Gum Simulator Infinity)")
        ...
        if not foundPriorityEgg then
            ...
            if totalServerTime > maxServerSearchTime and not eggFoundRecently then
                ...
                if hopToNextServer() then
                    ...
                    print("Server hop failed or was cancelled")
                    -- Reset timer anyway to prevent constant hop attempts
                    initialServerTime = tick()
                end
...
title.TextSize = 16
title.Text = "25x Priority Egg Finder v5.2"
title.Parent = frame
...
print("🚀 25x Priority Egg Finder v5.2 initialized")
