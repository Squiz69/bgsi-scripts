...
-- Improved button pressing function
local function pressPlayButtons()
    print("Looking for play buttons...")
    wait(3)
    
    -- Function to find and click buttons based on patterns
    local function clickButtonsByPattern(patterns, waitTime)
        if not LocalPlayer or not LocalPlayer:FindFirstChild("PlayerGui") then return false end
        
        -- Search all GUIs for buttons
        for _, gui in pairs(LocalPlayer.PlayerGui:GetDescendants()) do
            if gui:IsA("TextButton") or gui:IsA("ImageButton") then
                local buttonText = ""
                
                -- Get text content from the button
                if gui:IsA("TextButton") then
                    buttonText = gui.Text:lower()
                elseif gui:FindFirstChild("TextLabel") then
                    buttonText = gui:FindFirstChild("TextLabel").Text:lower()
                end
    ...
    local playButtonClicked = clickButtonsByPattern({"play", "start", "enter", "join"}, 1)
    
    -- Then try to click any quality/performance buttons
    if playButtonClicked then
        wait(1)
        clickButtonsByPattern({"optimized", "performance", "quality", "normal", "continue", "ok"}, 0.5)
    end
end

-- Optimized Discord webhook function
local function sendDiscordNotification(eggInfo, eggLuck)
    if not eggInfo or tick() - lastWebhookSent < webhookCooldown then return end
        ...
        embeds = {
            {
                ...
                footer = {text = "25x Priority Egg Finder v4.0"}
            }
        }
    }
...
-- Enhanced egg detector that prioritizes finding the priority egg
local function findLuckyEgg()
    print("Scanning for 25x lucky " .. egg_priority .. " egg...")
    
    -- Common egg-related patterns to check for
    local eggPatterns = {"egg", "pet", "capsule", "lootbox", "crate", "chest", "box", 
                        "prize", "reward", "drop", "gift", "present", "container"}
    
    -- Function to check if object name matches priority egg
    local function isPriorityEgg(obj)
        return string.find(string.lower(obj.Name), string.lower(egg_priority)) ~= nil
    end
    
    -- Function to scan for eggs recursively
    local function scanForEggs(parent, depth)
        ...
        for _, obj in pairs(parent:GetChildren()) do
            if obj ~= Players and obj.Name ~= "Players" then
                -- Check if object matches our priority egg
                local objNameLower = string.lower(obj.Name)
                local matchesPriority = isPriorityEgg(obj)
                
                -- Only proceed with checks if this matches our priority egg pattern
                if matchesPriority then
                    local luckValue = nil
                    ...
                    -- Check UI text for luck values
                    if not luckValue then
                        for _, child in pairs(obj:GetDescendants()) do
                            if child:IsA("TextLabel") or child:IsA("TextButton") then
                                local text = string.lower(child.Text)
                                
                                -- Common patterns for luck multipliers
                                local multiplierPatterns = {
                                    "(%d+)%s*x%s*luck", "x%s*(%d+)%s*luck", "luck%s*x%s*(%d+)",
                                    "(%d+)%s*times", "(%d+)%s*%%", "(%d+)%s*multiplier",
                                    "multi%s*(%d+)", "x(%d+)"
                                }
                    ...
                    -- If we found a priority egg with 25+ luck, return it immediately
                ...
                local childEgg, childLuck = scanForEggs(obj, depth + 1)
                if childEgg then
                    return childEgg, childLuck
                end
            end
        end
    ...
    -- Scan workspace first (highest priority)
    local priorityEgg, luckValue = scanForEggs(workspace, 0)
    if priorityEgg then
        return priorityEgg, luckValue
    end
    
    -- If priority egg not found in workspace, check ReplicatedStorage
    priorityEgg, luckValue = scanForEggs(ReplicatedStorage, 0)
    if priorityEgg then
        return priorityEgg, luckValue
    end
...
-- Streamlined tween movement function
...
local function openEgg(egg, amount)
    ...
    -- Try common remotes for egg opening
...
local function mainLoop()
    ...
    print("------- 25x Priority Egg Finder v4.0 Started -------")
...
title.TextSize = 16
title.Text = "25x Priority Egg Finder v4.0"
title.Parent = frame
...
print("🚀 25x Priority Egg Finder v4.0 initialized")
