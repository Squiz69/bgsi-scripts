...
local function sendWebhook(eggInfo, luckValue)
    ...
    local rarity = "25x Lucky"
    local shiny = false  -- Default, could be updated if we can detect this
    local image = fallbackImage
    local chance = "1/1000"  -- Default value, adjust if known
    
    -- Try to get total hatches from leaderstats if available
    local totalHatches = "Unknown"
    local hatches = player:FindFirstChild("leaderstats") and player.leaderstats:FindFirstChild("Total Hatches")
    ...
    -- Create webhook data
    local baseTitle = name .. " (" .. rarity .. ")"
    local desc = "Shiny: " .. (shiny and "Yes" or "No") .. "\nChance: " .. (chance or "N/A")
...
local function mainLoop()
    ...
    print("------- 25x Priority Egg Finder v5.0 Started -------")
    ...
    while wait(0.5) do
        ...
        if foundEgg then
            ...
            if not notifiedEggs[eggId] or (tick() - notifiedEggs[eggId]) > webhookCooldown then
                ...
                -- Reset timers ONLY if this was a priority egg - FIX FOR TIMER RESET ISSUE
                initialServerTime = tick()
                serverSearchTime = tick()
            end
        end
        
        -- FIXED: Only reset timer if we found a priority egg
        if not foundPriorityEgg then
            ...
            if totalServerTime > maxServerSearchTime and not eggFoundRecently then
                ...
                if hopToNextServer() then
                    wait(2)
                    -- FIXED: Make sure timers are reset properly after server hop
                    initialServerTime = tick()
                    serverSearchTime = tick()
                    notifiedEggs = {}
                else
                    -- FIXED: Reset timer even if server hop fails to prevent constant hopping attempts
                    initialServerTime = tick()
                end
...
title.TextSize = 16
title.Text = "25x Priority Egg Finder v5.0"
title.Parent = frame
...
spawn(function()
    while wait(15) do
        ...
        if currentTime - serverSearchTime > 20 and isSearching then
            ...
            initialServerTime = currentTime  -- FIXED: Reset initial server time
            wait(1)
            pcall(mainLoop)
        end
    end
...
print("🚀 25x Priority Egg Finder v5.0 initialized")
